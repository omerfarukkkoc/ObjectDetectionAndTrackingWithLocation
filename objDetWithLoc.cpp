...
//algılanan nesnenin x ve y lokasyonlarını tutacak değişkenler
  ...
  int eskiy = -1;
    ...
    while(true) //sürekli tarama yapmak için sonsuz döngü
    {
        ...
        //HSV'ye dönüştürülen frame Hmin-max,Smin-max,Vmin-max değer aralıklarına getirilerek isres nesnesine yüklendi     
        inRange(hsvFrame, Scalar(Hmin, Smin, Vmin), Scalar(Hmax, Smax, Vmax), islenenFrame);

        //HSV değerlerine göre asıl almak istenilen görüntünün tamamına ulaşmak için görüntü üzerinde 5x5'lik matrisle nokta belirginleştirme işlemi yapılıyor
        ...
        if(alan > 10000) //görüntünün alanı eşik değerinden büyükse 
        {
            ...
            if(eskix >= 0 && eskiy >=0 && yenix >= 0 && yeniy >= 0) //lokasyonlar sıfırdan büyükse
            {
                ...
                putText(yeniFrame, x +","+y, cvPoint(yenix,yeniy), 
        ...
        putText(yeniFrame," MAHSERIN 3 ATLISI ", cvPoint(2,20), 
